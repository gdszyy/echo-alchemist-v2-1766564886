...
> 弹珠是玩家直接操控的对象，每秒 60 次出现在屏幕上 → 是 Promare 美学密度最高的载体。

**体积构造（P2 三层堆叠）**

| 层 | 形状 | 尺寸 | 颜色 | 错位 |
|---|---|---|---|---|
| ① 最底（描边） | 等边三角 | r=10px | 主色暗调（粉系 #660F40 / 青系 #003E5C） | (0,0) |
| ② 中间（主体） | 等边三角 | r=8px | 元素主色（#FF2EA6 / #00B4FF / #FFE94A） | 偏移 (+1,+1) |
| ③ 顶层（高光） | 等边三角 | r=4px | 白 #FFFFFF 或 暖黄 #FFE94A | 偏移 (-2,-2) |

不是球，是**朝运动方向的三角剪影**。三角顶端 = `atan2(vel.y, vel.x)`。

**运动**

- **平移**：连续 60fps（物理需要，P4 例外）
- **旋转**：每 5 帧 snap +30°（顶端朝向跟速度方向，但量化到 12 个角度槽 = 每 30° 一档）
- **加速 trail**（speed > 8）：身后 2 个 afterimage 三角，size 0.7×/0.5×、alpha 0.5/0.25、位置后退 6px/12px、**stepped 切换**而非平滑滑动

**特殊元素**

- pyro 弹珠：trail 颜色叠加 1 层粉色 0.3 alpha + 顶层换成 1 个小黄三角
- cryo 弹珠：trail 替换为 2 个拉长菱形针
- lightning：trail 关掉，改为每 8 帧随机弹出 1 条 Z 字 stepped overlay
...
|---|---|---|---|---|---|
| **Ball** | 单色 △ 朝向 | 加速时 2 △ trail | 1 帧 white flash overlay | snap shrink (3 帧到 0) | 1 帧硬切显现 |
...
1. **弹珠是不是真的应该是 △ 不是 ○**？电影里 Burnish 火焰是三角，但 Promare 的"Promare 能源"球（Lio 召唤的能量球）是 ○。我倾向让 Ball = △（更具攻击意味），但如果你觉得 Lio 能源球更接近，可以改回 ○。
2. **boss_ouroboros（环 boss）排列成圆形**是否破坏"无 ○" 原则？我倾向用 8 个 □ 排成 ○ 的"形式 ≠ 语义"做法，但如果太矛盾可以改为"8 个 □ 排成正八边形"。
3. **当前 UI 菱形 clip-path** 是否要做 deprecation？菱形（4 顶点 □ 旋 45°）确实在敌人 vocabulary 里，UI 用它会把"我方面板"和"敌人结构"语义混淆。本设计建议 UI 改梯形，但工作量大。是否本轮做？
