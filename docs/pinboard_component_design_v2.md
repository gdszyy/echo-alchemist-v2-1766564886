...
> 实现状态：P0-D1 / P0-D2 已落地；P1 首批商店异形组件、编辑态轻量轮廓、放置覆盖预览、符文融合摘要、开始采集边界检查、初始 `2x5` 铺满与异形机关化已落地。
...
- 推荐默认：`5 + 5`，共 10 个小组件，解决初始漏球和异形组件不如常规交错阵的问题。
...
- 首发模块统一补“接缝钉”：左右边缘使用错位钉，上下边缘使用低/高位钉，让模块拼接后仍像连续钉阵。
- 接缝钉必须做近距保护，避免为了密度把钉距压到容易卡球。
- 多格异形模块只在外边缘补接缝钉，内部空间由模块自身路径设计负责。
...
总 Peg 目标：约 `157`；SpecialSlot 目标：`5`。异形组件必须至少提供路线物理、固定属性/融合优先级或真实 `SpecialSlot` 之一，避免弱于常规交错钉阵。
...
| ID | 名称 | 占位 | 分类 | 形态 | 机制 |
|---|---|---:|---|---|---|
| `guide_fin_left` | 左导流翼 | 1x1 | 导流 | 左高右低斜翼 | 约 15 颗钉，固定 `wind` 钉 + `pink` 翼形成导流身份 |
| `guide_fin_right` | 右导流翼 | 1x1 | 导流 | 右高左低斜翼 | 与左导流翼镜像，约 18 颗钉补足外侧空洞 |
| `split_lattice_bridge` | 分裂符文桥 | 2x1 | 分流/融合 | 宽桥 | 24 颗钉 + `split` 槽，左侧拆路、右侧融合承载 |
| `split_gate_light` | 轻分裂门 | 1x1 | 分流 | 窄门 | 12 颗钉夹住 `split` 槽，提供真实分裂收益 |
| `multicast_gate_light` | 连射门 | 1x1 | 奖励 | 沙漏门 | 约 17 颗钉夹住 `multicast` 槽，命中后追加连射 |
| `recall_loop_light` | 轻回环 | 1x1 | 回收 | 小回环 | 约 15 颗钉包住 `recall` 槽，降低漏球挫败 |
| `wheel_cup_light` | 轻轮盘杯 | 1x1 | 奖励 | 杯形 | 约 15 颗钉导向 `wheel` 槽，复制已收集属性 |
| `rune_lattice_light` | 轻符文格 | 1x1 | 聚焦/融合 | 小菱格 | 13 颗普通钉，中心 `fusionPriority=3`，承担初始密集融合目标 |
| `bounce_chamber` | 弹跳室 | 1x1 | 回收 | 圆角反弹腔 | 约 18 颗钉围出圆角反弹腔，3 颗 `pink` 维持强反馈 |
| `crucible_seed` | 炼金核 | 1x1 | 属性/融合 | 三角核 | 约 18 颗钉，固定候选属性钉 + 融合优先钉构成密集三角核 |
| `catcher_cup` | 接球杯 | 1x1 | 回收 | U 型杯 | 约 17 颗钉 + `recall` 槽，底部两侧 `pink` 减缓直落 |
...
- 1x1 首发组件 Peg 数量建议 `14 - 20`，其中包含接缝钉；商店普通 1x1 可低一些。
- 2x1 / 1x2 组件 Peg 数量建议 `18 - 30`，但必须有明确路线或机关收益。
- 2x2 组件 Peg 数量建议 `24 - 40`，且只放入稀有/史诗池。
...
默认布局直接使用上述 10 个组件，前两行铺满。现有 `rune_lattice`、`bouncer` 可保留为商店/旧存档兼容，不必删除。
...
- 初始 `5+5`：总 Peg 目标约 `157`，硬上限 `176`，SpecialSlot 不超过 `5`。
- 后期三行完整：总 Peg 目标约 `220`，硬上限 `250`，SpecialSlot 不超过 `8`。
