...
弹跳命中时在敌人表面产生 1 组短促绿色回弹弧 + 命中环（贴近命中点，不再拉成长线），约 0.28 秒内先放大强调，随后快速缩小淡出：
- PIXI.Graphics 画二次贝塞尔短弧，2 层 glow（外层 `#22c55e` alpha 0.20，内层薄荷色 `#dcfce7` alpha 0.46，透明度使用快速衰减曲线）
- 方向由子弹速度或命中点相对敌人中心决定，不再使用随机角度
- 多次弹跳在同一敌人上时，短弧组叠加（最多 3 组）
...
三阶共鸣：弹跳短弧变为双层回弹弧，并在命中点产生小型绿色扩散环。
