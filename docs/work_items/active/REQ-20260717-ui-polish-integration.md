...
状态：Goal Active — Implementing
负责人：Codex
开始日期：2026-07-22
最后更新：2026-07-22
当前里程碑：M2 集成冲突修复与共享测试
...
- 根 checkout 的战斗/药剂/规范/TODO 历史改动及未跟踪 `src/music_clip_packs.js` 不得被回退、复制进提交或静默吸收。干净模块图断链由专门提交 `2ec11ac` 的受控动态 import 修复，不激活来源/正典不明的 clip-pack loader。
...
- [ ] M2：完成 pause lease、术语、ARIA、CSS 状态、phase-contract 与共享 runner/README 集成修复。
- [ ] M3：所有语法检查、专项 T1、`validate_scenarios`、`validate_phase_contracts` 全绿。
- [ ] M4：T1 全绿后运行相关 T3，并在 360×800、390×844、480×854、桌面完成全链路浏览器回归。
- [ ] M5：索引、规则、TODO、umbrella/子卡、状态词、临时服务与 Git 全收口，形成集成交付提交。
...
- 2026-07-22：建立炼金台/符文配置/符文仓库/双碎片 canonical map、真理之书主解释与双向跳转；专项术语测试 `30/30`。
- 2026-07-22：快速集成回归通过：launcher/结算 `54/54`、生命周期 `109/109`、phase `175/175`、场景 `128/128`；全量 T1/T3 尚待后续里程碑。
- 2026-07-22：全量索引脚本扫描 36 个大文件，修复中央 `INDEX.md` / `_meta.json` 与合并树不一致；所有生成路径归本 integration 批次。

## 同步闸门

- Auto index：待合并后逐源文件生成并复核。
- 模块规则：待集成差异确认后同步受影响规则；不复制子卡已记录内容。
- 进度文档：`TODO.md`、P0 TODO、umbrella、A-D 子卡和本卡必须在完成时一致。
- Process insights：先复核 PI-006/007/008/012；只有发现新的可复用耦合才升版或新增。
- 资产/性能：本轮不生成资产、不新增粒子/渐变/混合/阴影；若集成修复触发性能规范，必须补 `@perf-impact` 与三档评估。
- 临时证据：截图/原始浏览器记录放在忽略目录或 Codex visualization 目录，不进入活跃源码路径。
