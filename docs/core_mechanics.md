...
2. **弹药继承优先级：** 由于跳过了研磨，本回合没有新的 `ammoQueue`。系统会**优先使用上回合保留的充能子弹（`_chargedAmmoQueue`）**作为本回合的弹药。如果连充能子弹也没有，则会尝试直接编译当前的 `marbleQueue`。
3. **直接进入战斗：** 完成弹药继承后，系统会清理所有替换上下文，直接进入战斗阶段，**不再触发子弹替换界面**。
