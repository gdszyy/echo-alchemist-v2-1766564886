...
游戏在 `rune_config.js` 中定义了一个套路克制关系字典（`COUNTER_MAP`）。例如：
- **火焰（pyro）：** 克制护盾（shield）和再生（regen）。
- **冰霜（cryo）：** 克制极速（haste）和跳跃（jump）。
- **闪电（lightning）：** 克制分身（clone）和治疗者（healer）。

系统将玩家的套路成分向量与克制关系字典相乘叠加，计算出一个“标签权重池（`tagWeights`）”。这个权重池反映了玩家当前流派最需要应对的敌人词缀类型。

### 3.3 符文关联与加权抽取

每个符文在数据库（`RUNE_DB`）中都有一个基础掉落权重（`baseDropWeight`）和一组亲和标签（`affinity_tags`）。
系统遍历所有符文，计算其最终权重：
**最终权重 = 基础掉落权重 + (亲和标签在权重池中的对应值 × 智能掉落乘数 3.0) + Boss 主题额外权重**

最后，系统使用加权随机算法（Weighted Random Choice）从候选池中抽取一个符文。这种机制确保了玩家更容易获得与其当前流派相契合、且能有效克制当前面临敌人的符文，从而提升了构筑（Build）的连贯性和策略深度。
