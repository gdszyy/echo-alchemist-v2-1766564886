...
本文档详细解析了《Echo Alchemist V2》中的几项核心游戏机制，包括弹珠包研磨、子弹充能与技能充能机制、遗物掉落保底机制、以及基于玩家套路的智能掉落算法。

## 1. 充能机制解析

游戏中的“充能”概念主要分为两类：**子弹充能（Ammo Charge）**与**技能充能（Skill Charge）**。
...
### 1.2 技能充能（Skill Charge）

技能充能是战斗阶段持续进行的 SP 获取过程，旨在奖励玩家的高频击中和击杀。它取代旧版“充能符文”奖励，充能满条后不再抽取或入库符文，而是直接尝试发放 1 个技能点（SP）。

**充能规则：**
- **基础充能：** 每次子弹击中敌人提供 3% 的充能值，如果造成击杀则提供 10% 的充能值，配置位于 `CONFIG.gameplay.skillChargeHitGain` 与 `skillChargeKillGain`。
- **概率翻倍：** 每次击中都有概率触发充能翻倍。具体概率为：1% 概率获得 8 倍充能，4% 概率获得 4 倍充能，13% 概率获得 2 倍充能，剩余 82% 为基础 1 倍充能。
- **实际条 / 临时条：** 每次获得的充能会按 `skillChargeRetainRatio` 拆分；默认 35% 进入实际条并永久保留，剩余 65% 进入临时条。
- **临时衰减：** 临时条在战斗中会以 `skillChargeDecayRate`（默认每帧 0.003）的速度衰减。实际条不衰减。
- **视觉表达：** UI 只显示一根连续充能槽；实际值表现为稳定底纹，临时值表现为槽内右侧会回落的半透明不稳定区域。

**SP 发放：**
当“实际条 + 临时条”的总值达到 100%（1.0）时，系统立刻尝试发放 1 个 SP，并消耗 1 条充能。若 SP 已达 `maxSkillPoints` 上限，则满条充能会以实际条形式保留，等玩家消耗 SP 后再尝试转换。
...
### 3.1 符文奖励敌人覆写（2026-06-23）

- `runeBearer` 敌人死亡时必定掉落 1 个通用智能符文，同时生成时和每个敌方回合开始拥有一个随机临时词条。
- `adaptiveRune` 敌人会记录最近承受的有效属性伤害或属性效果，死亡时通过 `forcedElement` 掉落该属性家族符文；没有记录时回退普通智能掉落。
- `loot_calcRuneDrop(game, overrideOptions)` 支持 `forcedElement` / `allowedElements` 候选池过滤，Boss 原有 `forcedLevel` / `themeWeights` 行为保持不变。

最后，系统使用加权随机算法（Weighted Random Choice）从候选池中抽取一个符文。这种机制确保玩家更容易获得与当前流派相契合的符文，从而提升构筑（Build）的连贯性。

## 3.4 符文法术与药剂炼成收口（2026-06-26）

符文组合与药剂炼成的当前权威规范见 [`docs/rune_potion_spell_contract.md`](rune_potion_spell_contract.md)。后续涉及 `RUNEWORD_DB` 法术化、`POTION_SPELL_DB`、药剂炼成顺序、法阵形态、嵌套合法性、防御塔构筑或坠击/扫射激光形态时，必须以该文档为准。

核心口径：符文组合生成法术内容；玩家绘制法阵决定形态与嵌套合约；药剂炼成是工作台草稿态，流程为“投入符文并立即消耗 -> 内部判定是否能形成法术 -> 绘制法阵 -> 手动接触封装成药剂，或继续投入符文配置下一层法术并查表校验复合嵌套”。最终生成前，玩家只能知道结构是否可生成/可继续，不知道具体会生成什么药剂。纯扣血、同类弹体套娃、链式反应、吸取/链接和未实现的场域不进入本版药剂嵌套。
