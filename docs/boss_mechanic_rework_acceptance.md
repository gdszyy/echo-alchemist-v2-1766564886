...
> 状态：执行中。2026-06-22 已落地第一批：Boss 专属敌人元数据、Boss 入场画像驱动转化、Ignis 温压流光护盾闭环；第二批：Tesla 导体网络、场强召唤闭环与 cryo / bounce 反制；第三批：Chimera 旧胃域吸引、吞噬继承状态与养料循环；2026-06-24 已将 Devourer / Chimera 吞噬机制互换，Devourer 接管胃域循环，Chimera 改为热核吞噬与核心转盾。
...
- [x] Chimera 每回合吞噬 2 名热核养料或温度目标：左侧倾向低温，右侧倾向高温，不再继承毒、冻结、相位失效等负面状态。
- [x] Chimera 吞噬温度敌人时按温度绝对值一比一获得热核/冰核，冷热抵消后按抵消层数 100% 转为流彩护盾；狂暴只让最终护盾量翻倍。
- [x] Chimera 每个正常行动结算后召唤 2-3 个不带 `berserk` 的 `thermalFeed` 养料，左侧低温、右侧高温。
- [x] 视觉显示 Devourer 胃域拉拽、Chimera 热核/冰核/流彩转盾累计。
...
- [x] 保存并读取后，Boss 机制状态不丢失、不重复触发：`validate_phase_contracts.mjs` 锁定 Boss 随从元数据、温压、孢甲资源、Tesla 场强、霜缝、Devourer 胃域冷却、Chimera 热核/冰核/转盾层与破绽状态的 save/load 成对恢复。
...
- [x] Chimera：能按左冷右热规则先吞噬两个温度目标；热核/冰核可抵消并按层数转为流彩护盾，狂暴后护盾量翻倍；吞噬结算后每个奇美拉回合生成 2-3 个不带狂暴词条的左冷右热热核养料。
...
5. Devourer / Chimera：Devourer 承接吸引、范围吞噬与非重叠召唤；Chimera 实现左冷右热双吞噬、热核/冰核抵消与流彩护盾转换。
...
- Chimera 的左右养料召唤最容易受占格影响，需要持续验证非重叠放置与左冷右热温度归属。
