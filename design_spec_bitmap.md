...
| 项目 | 规范 |
|---|---|
| 视觉关键词 | 双排履带、前置撞角、黄色警戒线、低频震动尘埃 |
| 材质语言 | 主体钢铁 (`#334155`) + 履带橡胶黑 (`#0c0a09`)、警戒条纹黄黑 (`#facc15`/`#000`) |
| 帧组 | idle 6 / move 4（迟缓）/ hit 3 / **siege_push 6**（重压推进特别帧） |
| 关键 Overlay | `enemy_siege_dust.png`（128×64，3 帧），`siege_push` 触发时在底部喷溅；`enemy_siege_warning_strip.png`（384×16）在重压前 1 回合闪烁警告 |
