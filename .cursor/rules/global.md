...
1. 执行 `git status --short --branch`，确认所有 modified / untracked 路径都有明确归属。
2. 修改文本文件后执行 `git diff --check`，避免行尾空格、换行和格式噪音混入。
3. 根据文件性质处理：
   - 当前任务交付物：保留并在总结中列出。
   - 生成索引：只允许通过 `scripts/generate_index.py` 生成。
   - 临时日志、截图、验证脚本、下载包：移动到忽略目录、删除，或在清理任务中用命名 stash 保存。
   - 运行时资产：同步 manifest / 资产清单 / TODO，否则不得作为无说明未跟踪文件遗留。
4. 若启动过 dev server，必须复用或关闭并记录端口、PID 与状态。
5. 最终回复必须包含 Git 状态、验证证据、同步闸门结果和仍需用户决策的事项。

### 清洁状态判定

| 状态 | 可接受条件 | 必须说明 |
|------|------------|----------|
| 干净 | `git status --short --branch` 无文件改动 | 分支是否领先远端、是否有 stash |
| 仅有本次预期改动 | 所有变更都属于当前任务且已说明 | 文件清单、验证、为何未提交 |
| 历史改动存在 | 变更早于当前任务或由用户/其他批次产生 | 不得回退；说明影响与后续处理建议 |
| 已 stash 清理 | 用户要求清洁或需要保护不明改动 | stash 名称、恢复命令、是否包含 untracked |

### 禁止事项

- 禁止把未跟踪文件当作“无关噪音”留到下一轮。
- 禁止把运行时资产、概念稿、下载资料和临时验证产物混在同一批未说明变更中。
- 禁止为了追求干净而直接 `git reset --hard`、批量删除或覆盖用户可能需要的成果。
- 禁止在没有用户要求提交时擅自用 commit 掩盖脏工作区；若需要完全干净且又要保留成果，应说明需要 commit、stash 或交由用户决策。
