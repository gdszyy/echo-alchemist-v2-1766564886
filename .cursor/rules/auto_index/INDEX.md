# 自动函数索引汇总 (Auto Index)

> 自动生成于 2026-04-22 | 由 code-indexer 脚本维护
> **本文件由脚本自动生成，严禁手动编辑。**

## 已索引文件 (29 个)

| 文件 | 总行数 | 函数数 | 巨型函数 | @section 标记 | 索引文件 |
|------|--------|--------|----------|--------------|---------|
| `src/entities.js` | 4834 | 108 | **4** | 21 | [src_entities_js_index.md](src_entities_js_index.md) |
| `src/entities/enemy.js` | 4793 | 26 | **3** | 18 | [src_entities_enemy_js_index.md](src_entities_enemy_js_index.md) |
| `src/combat_system.js` | 3362 | 42 | **5** | 22 | [src_combat_system_js_index.md](src_combat_system_js_index.md) |
| `src/systems.js` | 2810 | 85 | **2** | 10 | [src_systems_js_index.md](src_systems_js_index.md) |
| `src/game_phase.js` | 2493 | 16 | **4** | 15 | [src_game_phase_js_index.md](src_game_phase_js_index.md) |
| `src/game_system.js` | 2306 | 45 | **1** | 7 | [src_game_system_js_index.md](src_game_system_js_index.md) |
| `src/spawn_system.js` | 2230 | 28 | **2** | 8 | [src_spawn_system_js_index.md](src_spawn_system_js_index.md) |
| `src/effects/particles.js` | 1829 | 62 | **1** | 0 | [src_effects_particles_js_index.md](src_effects_particles_js_index.md) |
| `src/ui/rune_launcher.js` | 1702 | 25 | **1** | 0 | [src_ui_rune_launcher_js_index.md](src_ui_rune_launcher_js_index.md) |
| `src/config.js` | 1695 | 1 | **1** | 0 | [src_config_js_index.md](src_config_js_index.md) |
| `src/entities/projectile.js` | 1021 | 13 | **2** | 0 | [src_entities_projectile_js_index.md](src_entities_projectile_js_index.md) |
| `src/ui/hud.js` | 1019 | 14 | **1** | 0 | [src_ui_hud_js_index.md](src_ui_hud_js_index.md) |
| `src/ui_system.js` | 813 | 42 | **1** | 1 | [src_ui_system_js_index.md](src_ui_system_js_index.md) |
| `src/tutorial_system.js` | 811 | 16 | 0 | 0 | [src_tutorial_system_js_index.md](src_tutorial_system_js_index.md) |
| `src/rune_config.js` | 790 | 0 | 0 | 0 | [src_rune_config_js_index.md](src_rune_config_js_index.md) |
| `src/render_system.js` | 643 | 8 | 0 | 0 | [src_render_system_js_index.md](src_render_system_js_index.md) |
| `src/plinko_physics.js` | 597 | 15 | 0 | 0 | [src_plinko_physics_js_index.md](src_plinko_physics_js_index.md) |
| `src/audio.js` | 564 | 18 | 0 | 0 | [src_audio_js_index.md](src_audio_js_index.md) |
| `src/combat/damage_calc.js` | 460 | 7 | 0 | 0 | [src_combat_damage_calc_js_index.md](src_combat_damage_calc_js_index.md) |
| `src/ui/game_over.js` | 454 | 12 | 0 | 0 | [src_ui_game_over_js_index.md](src_ui_game_over_js_index.md) |
| `src/ui/shop.js` | 454 | 6 | 0 | 0 | [src_ui_shop_js_index.md](src_ui_shop_js_index.md) |
| `src/rune_system.js` | 444 | 9 | 0 | 0 | [src_rune_system_js_index.md](src_rune_system_js_index.md) |
| `src/combat/collision.js` | 443 | 3 | **1** | 0 | [src_combat_collision_js_index.md](src_combat_collision_js_index.md) |
| `src/core.js` | 418 | 5 | **1** | 0 | [src_core_js_index.md](src_core_js_index.md) |
| `src/calc_utils.js` | 332 | 8 | 0 | 0 | [src_calc_utils_js_index.md](src_calc_utils_js_index.md) |
| `src/loot_system.js` | 302 | 6 | 0 | 0 | [src_loot_system_js_index.md](src_loot_system_js_index.md) |
| `src/utils/math_utils.js` | 267 | 21 | 0 | 0 | [src_utils_math_utils_js_index.md](src_utils_math_utils_js_index.md) |
| `src/event_bus.js` | 258 | 13 | 0 | 0 | [src_event_bus_js_index.md](src_event_bus_js_index.md) |
| `src/combat/collision_shapes.js` | 146 | 2 | 0 | 0 | [src_combat_collision_shapes_js_index.md](src_combat_collision_shapes_js_index.md) |
