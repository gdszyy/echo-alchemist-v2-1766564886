...
> 最后更新：新增生物 `crystalDamageReduction`（晶石伤害减免比例）、`civilizationLevel`（聚落文明程度）与 `settlementSize`（聚落规模）属性；构造期会播种多样初始生物圈，部分高耐受物种会围绕未来 Alpha 区域生成。初始中心 Alpha 默认延迟 120 步播种，避免地幔能量尚未抵达中心时直接清空生物。Beta 采矿改为文明驱动的进度制，成功开采会使晶石周围一圈生物文明减半；地块低能塌陷消失机制已取消。稳定性研究入口收束为默认 50×50 环形前缘（ring-front），移除 source-basin/source-circle 双叶源井播种路径。本模块仍为纯数据子系统，**未接入** `Game` 主类、EventBus 或任何渲染层。
...
| `cell.js` | ~87 | `CELL_TYPE`、`createCell()` | 元胞工厂与状态枚举；含地形高度字段与 `mantleSourceStrength` 持久源井字段 |
| `params.js` | ~153 | `DEFAULT_PARAMS` | 约 80 个模拟参数（半径、能量、地形、温度、繁衍/变异系数、晶石源井阈值、人类吸能基地等）+ `SimulationParams` 契约 |
...
| `climate_layer.js` | ~115 | `climate_layer` | 气候层：地形感知的温度传导/对流/辐射冷却 + 地形抬升雷暴生成 |
...
| `bio_layer.js` | ~388 | `bio_layer` | 生物层：聚落与迁徙者的生存、繁衍、变异、地形感知迁移、灭绝；按物种晶石减伤结算 Alpha 邻接伤害 |
| `bio_spawn.js` | ~290 | `bio_spawn` | 生物投放：构造期初始生物圈、人类（speciesId=0）与随机新物种的选址、聚落规模与属性生成 |

所有文件均 < 500 行，均已登记进 [`auto_index/INDEX.md`](auto_index/INDEX.md)（因均未达大文件阈值，列于「未索引文件」段，不生成独立函数索引）。
...
| 气候 | `updateClimateLayer` | （单段） | 地形加权热传导 → 高海拔削弱地幔加热 + 海拔降温 → 地形削弱迎风对流 → 盆地热滞留 → 地形抬升雷暴生成 |
...
`updateBioLayer` 采用**先收集后应用**：遍历途中所有改动 push 进 `changes[]`，遍历结束后在 `@section:bio_apply_changes` 统一落盘。这样可避免遍历过程中改写网格、使后续元胞读到「半更新」状态。`changes` 的 `type` ∈ `STATE | PROSPERITY | MINING_STATE | NEW_BIO | MIGRATE | MIGRANT_ADD | MIGRANT_UPDATE | MIGRANT_REMOVE`。新增生物层行为时，**必须沿用此队列模式**，不得在遍历主循环里直接改 `cell.crystalState`。
...
*   **维护要求**：生物层只读取地形字段并沿用 `changes[]` 队列；气候层只读取地形字段并产出温度/雷暴，不得反写地形；只有地幔层和初始化播种器能维护 `terrainElevation` 及其派生指标。
...
*   成功开采的 Beta 会对其 8 邻域内所有 BIO 聚落施加文明损耗，`civilizationLevel *= betaMiningCivilizationDecay`，默认保留 50%。
*   采矿进度、文明损耗与晶石状态变更必须继续走 `changes[]` 队列，不得在生物层遍历主循环中直接改网格。

### 4.4 人类地幔能量汲取（human_energy.js）
...
### 4.5 晶石灾害拓扑分析（crystal_disaster.js）
...
*   **晶石减伤验证**：手工构造两个同等繁荣度聚落，分别给 0 与 0.8 的 `crystalDamageReduction`，断言高减伤聚落在 Alpha 邻接伤害后保留更多繁荣度。
*   **Beta 采矿验证**：低文明聚落只积累部分开采进度，高文明聚落一回合可完成一块 Beta；开采完成后该 Beta 周围一圈 BIO 文明降低 50%。
...
*   **地形迁徙与天气验证**：手工构造高原-平原-中坡走廊，断言迁徙者不会跨越陡崖而会选择唯一可通行走廊；同等地幔能量下，高海拔格从地幔获得的升温幅度必须低于低海拔格。
