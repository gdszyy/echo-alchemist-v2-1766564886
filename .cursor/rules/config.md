...
| 日期 | 文件 | 修改内容 |
|------|------|----------|
| 2026-06-19 | `src/config.js`, `src/ui_system.js`, `src/ui/shop.js`, `.cursor/rules/config.md` | **新增测试用局外购买项**：`debug_pick_any_relic` 归入局外商店「测试工具」分类，价格为 0；购买后打开全遗物池选择界面，可任选一个遗物并复用 `ui_selectRelic()` 的既有效果处理链路。 |
