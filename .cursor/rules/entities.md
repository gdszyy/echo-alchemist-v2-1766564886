...
| 日期 | 文件 | 修改内容 |
|------|------|----------|
| 2026-06-19 | `src/entities/enemy.js`, `src/config.js` | **敌人受击闪白降噪**：将敌人命中后的主体白色覆盖从硬编码 `4` 帧、最高 `0.8` alpha 改为 `CONFIG.enemyRender.hitFlashDuration = 2` 与 `hitFlashMaxAlpha = 0.16` 驱动。普通受击与燃烧跳伤统一读取该配置，保留短促命中反馈，但不再把敌人主体背景持续刷白，避免削弱浮动扣血数字和血条变化的可读性。性能自适应影响评估：仅降低既有 `fillRect` 覆盖层透明度和持续时间，未新增 `shadowBlur`、`createRadialGradient`、`screen/lighter` 混合或粒子；high/medium/low 三档开销均小幅下降，不需要新增 `CONFIG.performance` 预算字段。 |
