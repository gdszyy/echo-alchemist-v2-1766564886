...
| 2026-06-24 | `src/entities/enemy.js`, `src/config.js`, `src/game_system.js`, `src/systems.js`, `tests/validate_enemy_spawn_runtime.mjs` | **Devourer / Chimera 吞噬机制互换 + Glacies 霜缝提示增强**：Devourer 接管胃域拉拽、`maw_thrall` 召唤与范围吞噬，吞噬胃域内非 Boss 敌人并转化为护盾，狂暴后全屏候选；Chimera 改为随机热核吞噬，吞噬目标温度转热/寒层，冷热抵消后通过流彩护盾脉冲换盾，狂暴时本次层数翻倍，并在吞噬后召唤 +100°C / -100°C `thermalFeed`。Glacies 霜缝目标状态短标显示减伤比例，并绘制冰框、斜向缝线和减伤字样。性能自适应影响评估：新视觉均复用既有描边、floating text、shockwave、mist/spark/shard 预算；`shadowBlur` 仍由 `_sb()` 和 low 档门控，不新增持续粒子池或 `CONFIG.performance` 字段。 |
