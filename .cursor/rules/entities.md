...
| **Layer 3.8b** | Boss 破绽身体 Overlay | **2026-06-22 新增**：`_drawBossVulnerabilityOverlay()` 在 Boss 专属装饰后绘制破绽进度、属性痕迹、暴露部位、爆开和恢复状态；若 `src/data/boss_vulnerability_assets.js` 解析出的透明 PNG 已加载，则优先绘制 PNG，缺失时回落到 Canvas 线稿。`low` 档 fallback 为平面线条，high/medium 使用少量 `_sb()` 门控发光。 |
| **Layer 3.8c** | Boss 本体 Sprite | **2026-06-23 更新**：`_drawBossDecoration()` 通过 `src/data/boss_sprite_assets.js` 优先绘制 `assets/sprites/bosses/redraw_drafts/boss_<bossId>_redraw_idle_draft_sheet.png`；旧 `assets/sprites/bosses/boss_<bossId>.png` 仅保留为 legacy 对照。`384 × 256` 横向帧按 3×2 Boss 占格居中绘制。Sprite 只承担本体外观，破绽、血条、温度、狂暴、行动预警仍由上层 Canvas/Overlay 负责。 |
