 ...
 *
 * @param {Array<string|null>} grid - 长度为 9 的数组，表示 3x3 网格（索引 0~8 对应左上到右下）
 *   索引布局：
 *     0 | 1 | 2
 *     3 | 4 | 5
 *     6 | 7 | 8
...
function parseRuneGrid(grid, runewordDb) {
    ...
    const pathSequences = PATHS.map(path => ({
        indices: path,
        runes: path.map(i => grid[i]).filter(r => r !== null && r !== undefined),
    }));
    ...
    for (const runeword of runewordDb) {
        ...
        for (const { indices, runes } of pathSequences) {
            ...
            // 在路径的符文序列中查找正向或反向匹配
            const matched = findPatternInSequence(runes, pattern, indices, grid);
            if (matched) {
                // 避免同一词条重复激活
 ...
 * @param {Array<string|null>} grid - 完整的 9 格网格
