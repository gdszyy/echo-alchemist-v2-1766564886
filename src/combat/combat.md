...
- Tower runtime: `combat_spawnPotionTower()` creates `kind: 'potion_tower'` with hp, lifetime, radius, slot type, pulse timer, pulse interval, hitbox, and contact-damage cooldown. `combat_updatePotionTower()` handles ticking, AABB blocking, contact damage, active-slot pulses, and death/expiry. Active towers pulse periodically; death towers stay dormant until destroyed, then release once through `combat_destroyPotionTower()`.
- Tower blocking: `combat_getPotionTowerHitbox()` defines the construct AABB; touching enemies are clamped above the tower via `pos.y/dropTargetY` and damage the tower on a cooldown through `combat_applyPotionTowerBlocker()`. Damage reuses `Enemy._getDefenseBarrierDamage(game)` when available, with a footprint + `siegeBreaker` fallback for tests and compatibility objects.
- Tower targeting: `combat_selectPotionTowerTargets()` filters by tower radius, then sorts nearest-first with stable `y/x` tie-breaks. Active towers target one enemy per pulse; death towers use their wider release target limit.
...
- Shared legality comes from `src/potion_nesting.js`. Combat must reject illegal trees before spawning carriers/towers; direct tower spawning also revalidates the root through the shared helper. Tower must not spawn Tower, active/death slots must stay mutually exclusive, and invalid trees must not consume charges or create carrier/tower runtime objects.
