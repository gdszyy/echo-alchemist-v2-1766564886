...
> [!IMPORTANT]
> **版本声明**: 本仓库目前已回归并专注于原生的 JavaScript + HTML5 Canvas 版本开发。之前尝试的 Godot 4.x 迁移版本由于架构和类型断层问题，已移至 `docs/archive/godot_migration/` 目录进行归档封存。
...
| **`src/combat_system.js`** | 战斗系统逻辑。 | 伤害计算、碰撞反馈、元素连击及特殊攻击判定。 |
| **`src/spawn_system.js`** | 实体生成系统。 | 敌人、弹丸、掉落物、粒子及词缀生成。 |
| **`src/rune_system.js`** | 符文词条系统。 | 符文网格解析、词条匹配及掉落计算。 |
...
### 2.5 符文词条系统 (Rune System)
全新的符文系统，增加了游戏的策略深度：
- **智能掉落**: 基于玩家当前套路成分识别和克制关系映射，动态计算符文掉落权重。
- **网格镶嵌**: 玩家可以在 3x3 的发射器网格中镶嵌符文，通过不同的排列组合激活强力词条。
