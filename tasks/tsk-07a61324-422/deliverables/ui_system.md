...
| 日期 | 文件 | 问题描述 | 修复方式 |
|---|---|---|---|
| 2026-04-17 | `index.html`, `src/ui_system.js`, `src/spawn_system.js`, `src/ui/shop.js`, `src/game_system.js` | **命运时刻 / 纯净精华 UI 接入**：原选择阶段底栏将数量硬编码为 `0 / 3`，遗物 overlay 关闭后也默认重建旧 selection 流程，导致纯净精华单弹珠模式和命运时刻 overlay 返回场景都无法正确工作。 | 1. `index.html` 将选择阶段文案改为 `selection-mode-label / selected-required-count / selection-mode-subtitle` 三段动态结构；2. `ui_system.js` 新增 `ui_getSelectionRequirement`、`ui_isSelectionConfirmReady`、`ui_renderPureEssencePanel`、`ui_refreshSelectionModeUI`，统一控制单弹珠模式、合法符文注入与确认按钮启用条件；3. `spawn_system.js` 在预览面板中接入注入 UI，并显示双倍同化率提示；4. `shop.js` 关闭遗物界面时按 `relicOverlayReturnState` 返回原阶段，不再一律回退旧 selection 初始化。 |
...
5. 若功能影响选择阶段底栏文案或确认条件，**禁止再写死 `3`**，必须统一走 `selectionRequiredCount` / `ui_getSelectionRequirement()`。
6. 若功能属于命运时刻 overlay（遗物、轮盘、纯净精华注入前置），关闭 overlay 时必须恢复来源阶段，不得默认重建 `sys_initSelectionPhase()` 覆盖当前上下文。
...
| `ui_getSelectionRequirement()` | 读取当前选择阶段需要的弹珠数量（标准 3 枚 / 纯净精华 1 枚） |
| `ui_isSelectionConfirmReady()` | 判断当前模式下确认按钮是否可用 |
| `ui_getPureEssenceLegalElements(marbleDef)` | 计算纯净精华允许注入的合法属性集合 |
| `ui_getPureEssenceRuneOptions(marbleDef)` | 从符文库存中过滤出当前弹珠可合法注入的符文 |
| `ui_selectPureEssenceRune(selectionIndex, inventoryIndex)` | 选中用于注入的合法符文 |
| `ui_renderPureEssencePanel(marbleDef, selectionIndex)` | 在弹珠预览面板中渲染纯净精华注入 UI |
| `ui_refreshSelectionModeUI()` | 刷新命运抉择 / 纯净精华模式标题、数量和按钮状态 |
