...
export const game_system = {
    ...
    sys_initSelectionPhase() {
        ...
        const modeConfig = this.pendingSelectionMode || null;
        this.selectionMode = modeConfig?.mode || 'standard';
        this.selectionRequiredCount = modeConfig?.requiredCount || CONFIG.gameplay.selectionReq || 3;
        this.pendingSelectionMode = null;
        this.selectionInjectedRune = null;
        this.selectionPreviewState = null;
        this.relicOverlayReturnState = { phase: 'selection', mode: this.selectionMode };
    ...
    sys_toggleMarbleSelection(idx, cardEl) {
        const limit = Math.max(1, this.selectionRequiredCount || CONFIG.gameplay.selectionReq || 3);
        if (this.selectedMarbles.includes(idx)) {
            this.selectedMarbles = this.selectedMarbles.filter(i => i !== idx);
            cardEl.classList.remove('selected');
        } else {
            if (limit === 1 && this.selectedMarbles.length === 1) {
                const prevIdx = this.selectedMarbles[0];
                const prevCard = document.querySelector(`.select-card[data-marble-index="${prevIdx}"]`);
                if (prevCard) prevCard.classList.remove('selected');
                this.selectedMarbles = [];
            }
            if (this.selectedMarbles.length < limit) {
        ...
        if (this.selectionMode === 'pure_essence') {
            const selectedIdx = this.selectedMarbles[0];
            if (!this.selectionInjectedRune || this.selectionInjectedRune.marbleIndex !== selectedIdx) {
                this.selectionInjectedRune = null;
            }
        }
        ...
        if (typeof this.ui_refreshSelectionModeUI === 'function') this.ui_refreshSelectionModeUI();
        // 通知教程系统：已达到当前模式要求的弹珠数量
        if (count === limit) eventBus.emit('tutorial:marbles_ready');
    },
    ...
    sys_loadRunState() {
        ...
        try {
            ...
            this.doubleAssimilationBoostRounds = { ...(state.doubleAssimilationBoostRounds || {}) };
