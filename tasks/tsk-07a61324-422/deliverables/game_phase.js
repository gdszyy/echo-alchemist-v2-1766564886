    ...
    phase_handleInputStart(pos) {
        ...
        if (this.phase === 'combat') {
            ...
            if (pos.y < this.height * 0.85) {
                ...
                if (Array.isArray(marbleDef.collected) && marbleDef.collected.length > 0) {
                    marbleDef.collected.forEach(item => {
                        this.currentSession.collected.push(typeof item === 'string' ? item : { ...item });
                    });
                }
                if (marbleDef.type === 'laser' && !this.currentSession.collected.includes('laser')) {
                    this.currentSession.collected.push('laser');
                } else if (marbleDef.type === 'colored' && marbleDef.type && !this.currentSession.collected.includes(marbleDef.type)) {
            ...
            for (const mt of Object.keys(this.doubleAssimilationBoostRounds)) {
                if (this.doubleAssimilationBoostRounds[mt] > 0) {
                    ...
                    if (this.doubleAssimilationBoostRounds[mt] === 0) {
                        ...
                        showToast(`${display} 的同化率 x${CONFIG.gameplay.assimilationDoubleMultiplier || 2} 效果已結束。`);
                    }
                }
            }
        }
