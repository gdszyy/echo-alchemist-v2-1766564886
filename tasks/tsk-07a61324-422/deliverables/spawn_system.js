...
/**
    ...
    spawn_generateMarbleOptions() { 
        ...
        for(let i=0; i < CONFIG.gameplay.selectionCount; i++) {
            ...
            card.onclick = () => {
                ...
                this.spawn_showMarblePreview(m, tbEntry, supplementDesc, i);
            };
        ...
        if (this.marblesPool.length > 0) {
            ...
            this.spawn_showMarblePreview(firstM, firstTbEntry, supplementDesc, 0);
        }
    },
     ...
     */
    spawn_showMarblePreview(m, tbEntry, supplementDesc, selectionIndex = null) {
        ...
        let pegEffect = pegEffects[m.type] || '可同化普通鉤釘，使其轉化為對應屬性。';
        if (this.doubleAssimilationBoostRounds && this.doubleAssimilationBoostRounds[m.type] > 0) {
            pegEffect += `\n\n當前命運增益：同化率 x${CONFIG.gameplay.assimilationDoubleMultiplier || 2}（剩餘 ${this.doubleAssimilationBoostRounds[m.type]} 回合）。`;
        }
        ...
        this.selectionPreviewState = { marble: m, tbEntry, supplementDesc, selectionIndex };
        if (typeof this.ui_renderPureEssencePanel === 'function') {
            this.ui_renderPureEssencePanel(m, selectionIndex ?? m._selectionIndex ?? -1);
        }
