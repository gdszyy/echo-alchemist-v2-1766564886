...
- **变体模式（2026-04-17）**：当玩家获得 **纯净精华** 时，下一次命运抉择不再要求选择 3 枚弹珠，而是切换为 **只选 1 枚弹珠 + 注入 1 个合法符文** 的单弹珠模式。该模式由 `selectionMode='pure_essence'` 与 `selectionRequiredCount=1` 驱动，且确认按钮只有在「已选中 1 枚弹珠且已完成合法符文注入」时才能启用。
...
- **命运时刻命名**: 固定回合遗物事件的提示文案已统一为 **命运时刻**。其中 **混沌精华** 继续复用旧版 `FortuneWheel` / `slotType: 'wheel'` 逻辑，不得另起一套平行命运轮盘实现。
...
- `ui_closeRelicSelection` 中的串行遗物分支已移除，但**不能**再简单地一律回到 `sys_initSelectionPhase()`。
- **当前约束（2026-04-17）**：遗物/命运 overlay 打开时必须记录 `relicOverlayReturnState`，关闭时按来源阶段恢复：
  - 若来源为 `gathering`，调用 `phase_gathering_attemptComplete()` 继续当前收尾流程；
  - 若来源为 `selection`，通过 `phase_switchPhase('selection') + ui_updateUI()` 返回当前选择界面，**禁止**默认重建旧的 selection 流程；
  - 仅在无法识别来源时才回退到 `sys_initSelectionPhase()` 兜底。
