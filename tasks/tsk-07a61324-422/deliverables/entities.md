...
## 4.1 命运时刻相关实体约束（2026-04-17）
*   **混沌精华继续复用旧轮盘实体**：命运时刻中的「混沌精华」并未引入新轮盘类，仍然通过 `SpecialSlot(type='wheel')` + `FortuneWheel` 承载旧版命运轮盘逻辑。若未来继续扩展该遗物，优先修改现有 `FortuneWheel.spin()` 与槽位触发链路，禁止复制一份平行轮盘实现。
*   **双倍同化率显式字段**：同化涌潮类遗物现在使用 `doubleAssimilationBoostRounds` 表示剩余回合，`entities.js` 在普通钉子同化判定时显式读取该字段并按 `CONFIG.gameplay.assimilationDoubleMultiplier` 倍率放大概率，禁止再写回匿名魔法常数。
*   **纯净精华注入落点**：纯净精华对弹珠的注入结果统一回写到 `MarbleDefinition.collected`，随后由 `game_phase.js` 在弹珠发射时灌入 `currentSession.collected`。这意味着实体层只消费标准 `collected` 数据结构，不感知 UI 是否来自纯净精华模式。

## 5. 参数调整记录

| 日期 | 文件 | 修改内容 |
|------|------|----------|
| 2026-04-17 | `src/entities.js`, `src/game_phase.js`, `src/config.js`, `.cursor/rules/entities.md` | **命运时刻实体链路补充（Chaos / Pure Essence）**：1. 继续沿用 `FortuneWheel` 作为混沌精华的旧版命运轮盘实体实现，不引入重复轮盘类；2. 在 `config.js` 中新增 `assimilationDoubleMultiplier`，并将涌潮效果改为显式的「同化率 x2」；3. 在 `entities.js` 的普通钉子同化判定中改读 `doubleAssimilationBoostRounds`，替代原先匿名概率常数；4. 在 `game_phase.js` 中让 `MarbleDefinition.collected` 能作为纯净精华注入后的实体输入，发射时一并灌入 `currentSession.collected`。 |
