...
| PI-006 | 命运时刻 Overlay 返回流与纯净精华选择模式 | v1.0 | ui/shop, game_system, ui_system, spawn_system, game_phase | 2026-04-17 | [PI-006_destiny_overlay_return_and_selection_mode.md](PI-006_destiny_overlay_return_and_selection_mode.md) |

### 按模块快速检索

| 模块 | 相关洞察 |
|------|---------|
| `game_phase.js` | PI-001（阶段切换双重赋值、round++ 重复执行） |
| `ui_system.js` | PI-001（multicast 颜色顺序、setDeepValue 双重调用） |
| `game_system.js` | PI-001（specialSlots 初始化类型）、PI-003（sys_resetGame 新属性重置） |
...
| `spawn_system.js` | PI-004（EnergyOrb 聚合优化）、PI-006（纯净精华预览与注入面板挂载） |
| `effects/particles.js` | PI-004（未接入预算的高风险特效清单）、PI-005（性能自适应影响评估） |
| `render_system.js` | PI-005（性能自适应影响评估） |
| `ui/shop.js` | PI-006（命运时刻 overlay 返回流） |
