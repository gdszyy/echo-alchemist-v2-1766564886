...
| 模块 | 相关洞察 |
|------|---------|
| `game_phase.js` | PI-001（阶段切换双重赋值、round++ 重复执行）、PI-006（回合结束后必须进入 round-start resolver） |
| `ui_system.js` | PI-001（multicast 颜色顺序、setDeepValue 双重调用） |
| `game_system.js` | PI-001（specialSlots 初始化类型）、PI-003（sys_resetGame 新属性重置）、PI-006（pendingRoundStartRewards 存档/恢复） |
...
| `core.js` | PI-003（_subsystems 数组、组合模式）、PI-006（enemy:killed 只登记延迟奖励） |
